...
/obj/structure/mark_turf
	name = "turf"
	icon = 'overrides/icons/effects/turf_effects.dmi'
...
/obj/structure/mark_turf/Initialize(mapload, current_turf)
	...
	switch(current_turf)
		...
		if("water")
			...
			icon_state = "water"
			src.add_overlay(image('overrides/icons/effects/turf_effects.dmi', "water_top", EXTRA_ABOVE_MOB_LAYER))
		...
		if("smoke")
			...
			icon_state = "smoke"
			src.add_overlay(image('overrides/icons/effects/turf_effects.dmi', "smoke_top", EXTRA_ABOVE_MOB_LAYER))
		...
		if("holoseat")
			...
			icon_state = "holoseat"
			src.add_overlay(image('overrides/icons/effects/turf_effects.dmi', "holoseat_top", EXTRA_ABOVE_MOB_LAYER))
			playsound(get_turf(src), 'sound/misc/compiler-stage2.ogg', 25, TRUE)
		...
		if("slime")
			...
			switch(rand(1,1000))
				if(-INFINITY to 400)
					...
					src.add_overlay(image('overrides/icons/effects/turf_effects.dmi', "slimeobj1_top", EXTRA_ABOVE_MOB_LAYER))
				if(400 to 800)
					icon_state = "slimeobj2"
					src.add_overlay(image('overrides/icons/effects/turf_effects.dmi', "slimeobj2_top", EXTRA_ABOVE_MOB_LAYER))
				if(800 to 980)
					icon_state = "slimeobj3"
					src.add_overlay(image('overrides/icons/effects/turf_effects.dmi', "slimeobj3_top", EXTRA_ABOVE_MOB_LAYER))
				if(980 to INFINITY)
		...
		if("dust")
			...
			icon = 'overrides/icons/effects/turf_effects_64.dmi'
			icon_state = "dust"
			pixel_x = -16
			src.add_overlay(image('overrides/icons/effects/turf_effects_64.dmi', "dust_top", EXTRA_ABOVE_MOB_LAYER))
		...
		if("tails")
			...
			var/mutable_appearance/overlay = mutable_appearance('overrides/icons/effects/turf_effects_64.dmi', "tails_top", EXTRA_ABOVE_MOB_LAYER, src, GAME_PLANE_UPPER)
		...
		if("constrict")
			...
			icon_state = "naga"
			pixel_x = -16
			var/mutable_appearance/overlay = mutable_appearance('overrides/icons/effects/turf_effects_64.dmi', "naga_top", EXTRA_ABOVE_MOB_LAYER, src, GAME_PLANE_UPPER)
			overlay.appearance_flags = TILE_BOUND|PIXEL_SCALE|KEEP_TOGETHER
			src.add_overlay(overlay)
