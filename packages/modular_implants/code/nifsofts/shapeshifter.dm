...
/datum/action/innate/alter_form/nif/change_form(mob/living/carbon/human/alterer)
	var/selected_alteration = show_radial_menu(
		...
		alterer,
		list(
			"DNA" = image(icon = 'packages/customization/assets/mob/actions/actions_slime.dmi', icon_state = "dna"),
			"Hair" = image(icon = 'packages/customization/assets/mob/actions/actions_slime.dmi', icon_state = "scissors"),
			"Markings" = image(icon = 'packages/customization/assets/mob/actions/actions_slime.dmi', icon_state = "rainbow_spraycan"),
		),
		tooltips = TRUE,
	)
