...
/turf/closed/indestructible/fakeglass/Initialize(mapload)
	. = ..()
	underlays += mutable_appearance(EFFIGY_TURFS_ICON_FILE, "grille", layer - 0.01) //add a grille underlay
	underlays += mutable_appearance(EFFIGY_TURFS_ICON_FILE, "plating", layer - 0.02) //add the plating underlay, below the grille
...
/turf/closed/indestructible/opsglass/Initialize(mapload)
	...
	underlays += mutable_appearance(EFFIGY_TURFS_ICON_FILE, "plating", layer - 0.02)
...
/turf/closed/indestructible/resin/membrane/Initialize(mapload)
	. = ..()
	underlays += mutable_appearance(EFFIGY_TURFS_ICON_FILE, "engine") // add the reinforced floor underneath
...
/turf/closed/indestructible/grille/Initialize(mapload)
	. = ..()
	underlays += mutable_appearance(EFFIGY_TURFS_ICON_FILE, "plating")
