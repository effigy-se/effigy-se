...
/obj/item/reagent_containers/cup/bucket/attackby(obj/O, mob/living/user, params) // EffigyEdit Change
	if(istype(O, /obj/item/mop))
		...
		if(is_right_clicking)
			...
			user.balloon_alert(user, "squeeze [O] to [src]")
		else
			if(reagents.total_volume < 1)
				user.balloon_alert(user, "out of water!")
			else
				reagents.trans_to(O, 5, transfered_by = user)
				user.balloon_alert(user, "sloshed")
