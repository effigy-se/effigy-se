...
/obj/machinery/plumbing/pill_press
	...
	var/max_bottle_volume = 60
	///maximum size of a vial
	var/max_vial_volume = 60 // EffigyEdit Add
...
/obj/machinery/plumbing/pill_press/process(seconds_per_tick)
	...
	if(stored_products.len)
		...
		for(var/thing in loc)
			if(!istype(thing, /obj/item/reagent_containers/cup/bottle) && !istype(thing, /obj/item/reagent_containers/pill) && !istype(thing, /obj/item/reagent_containers/cup/hypovial)) // EffigyEdit Change (Medical)
				continue
			pill_amount++
