...
/datum/reagent/flightpotion/proc/get_wing_choice(mob/living/carbon/human/needs_wings)
	...
	for(var/obj/item/organ/external/wings/functional/possible_type as anything in wing_types)
		...
		accessory = GLOB.sprite_accessories[initial(accessory.key)][initial(accessory.name)] // EFFIGY EDIT CHANGE (#3 Customization - Ported from Skyrat): accessory = GLOB.wings_list[initial(accessory.name)] //get the singleton instance
