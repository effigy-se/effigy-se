/mob/living/carbon/human/Initialize(mapload)
	...
	AddElement(/datum/element/footstep, FOOTSTEP_MOB_HUMAN, 0.6, -6) // EffigyEdit Change
...
/mob/living/carbon/human/assess_threat(judgement_criteria, lasercolor = "", datum/callback/weaponcheck=null)
	...
	// if(dna && dna.species.id && dna.species.id != SPECIES_HUMAN) // EffigyEdit Remove
	//	threatcount += 1 // EffigyEdit Remove
...
/mob/living/carbon/human/update_health_hud()
	...
	for(var/obj/item/bodypart/body_part as anything in bodyparts)
		...
		if(has_status_effect(/datum/status_effect/grouped/screwy_hud/fake_healthy))
			icon_num = 0
		if(icon_num)
			hud_used.healthdoll.add_overlay(mutable_appearance(GENERAL_SCREEN_ICONS, "[body_part.body_zone][icon_num]")) // EffigyEdit Change - Custom HUD
	for(var/t in get_missing_limbs()) //Missing limbs
		hud_used.healthdoll.add_overlay(mutable_appearance(GENERAL_SCREEN_ICONS, "[t]6")) // EffigyEdit Change - Custom HUD
	for(var/t in get_disabled_limbs()) //Disabled limbs
		hud_used.healthdoll.add_overlay(mutable_appearance(GENERAL_SCREEN_ICONS, "[t]7")) // EffigyEdit Change - Custom HUD
...
/mob/living/carbon/human/species/set_species(datum/species/mrace, icon_update = TRUE, pref_load = FALSE, list/override_features, list/override_mutantparts, list/override_markings, retain_features = FALSE, retain_mutantparts = FALSE) // EffigyEdit Change
	. = ..()
	if(use_random_name)
