	...
	if(!HAS_TRAIT(species_human, TRAIT_NO_UNDERWEAR))
		if(species_human.underwear && !(species_human.underwear_visibility & UNDERWEAR_HIDE_UNDIES))
			...
			if(underwear)
				...
				if(underwear.has_digitigrade && (species_human.bodytype & BODYSHAPE_DIGITIGRADE))
					icon_state += "_d"
		...
		if(species_human.socks && species_human.num_legs >= 2 && !(species_human.bodyshape & BODYSHAPE_DIGITIGRADE) && !(mutant_bodyparts["taur"]) && !(species_human.underwear_visibility & UNDERWEAR_HIDE_SOCKS))
			var/datum/sprite_accessory/socks/socks = GLOB.socks_list[species_human.socks]
			if(socks)
				var/mutable_appearance/socks_overlay
				var/icon_state = socks.icon_state
				if((species_human.bodytype & BODYSHAPE_DIGITIGRADE))
					icon_state += "_d"
...
/* EffigyEdit Remove - moved to local/code/modules/mob/living/human/species.dm
