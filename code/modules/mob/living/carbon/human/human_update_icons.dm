...
/mob/living/carbon/human/update_worn_undersuit()
	...
	if(istype(w_uniform, /obj/item/clothing/under))
		...
		else if(bodyshape & BODYTYPE_CUSTOM)
	...
	if(head)
		...
		if(bodytype & BODYTYPE_CUSTOM)
			...
			if(species_icon_file)
				icon_file = species_icon_file
				mutant_override = TRUE
		if((icon_file == 'icons/mob/clothing/head/default.dmi') && (bodytype & BODYSHAPE_SNOUTED) && (worn_item.supports_variations_flags & CLOTHING_SNOUTED_VARIATION))
...
/mob/living/carbon/human/update_worn_oversuit()
	...
	if(wear_suit)
		...
		if(bodytype & BODYSHAPE_DIGITIGRADE)
	...
	if(wear_mask)
		...
		if(bodytype & BODYTYPE_CUSTOM)
			...
			if(species_icon_file)
				icon_file = species_icon_file
				mutant_override = TRUE
		if((icon_file == 'icons/mob/clothing/mask.dmi') && (bodytype & BODYSHAPE_SNOUTED) && (worn_item.supports_variations_flags & CLOTHING_SNOUTED_VARIATION))
...
/proc/wear_female_version(t_color, icon, layer, type, greyscale_colors)
	...
	return mutable_appearance(GLOB.female_clothing_icons[index], layer = -layer, icon_state = t_color) // EffigyEdit Change - Taur-friendly uniforms and suits - Adds `icon_state = t_color`
...
mutant_styles: The mutant style - taur bodytype, STYLE_TESHARI, etc. // EffigyEdit AddITION - Taur-friendly suits and uniforms
*/
/obj/item/proc/build_worn_icon(
	...
	mutant_styles = NONE, // EffigyEdit Add - Further outfit modification for outfits (added `mutant_styles` argument)
)
	...
	var/list/worn_overlays = worn_overlays(standing, isinhands, file2use, mutant_styles) // EffigyEdit Change - ORIGINAL: var/list/worn_overlays = worn_overlays(standing, isinhands)
