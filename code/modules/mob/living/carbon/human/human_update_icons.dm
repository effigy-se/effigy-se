...
/mob/living/carbon/human/update_worn_undersuit()
	...
	if(istype(w_uniform, /obj/item/clothing/under))
		...
		if((dna?.species.bodytype & BODYTYPE_MONKEY) && (uniform.supports_variations_flags & CLOTHING_MONKEY_VARIATION))
			...
			icon_file = uniform.worn_icon_digi || DIGITIGRADE_UNIFORM_FILE // EffigyEdit Change

		// EffigyEdit Add -  (#3 Customization - Ported from Skyrat) birbs
		...
		// EffigyEdit Add -  (#3 Customization - Ported from Skyrat) Taur-friendly suits!
...
/mob/living/carbon/human/update_worn_glasses()
	...
	if(glasses)
		...
		// EffigyEdit Add -  (#3 Customization)
		var/mutant_override = FALSE
		if(dna.species.bodytype & BODYTYPE_CUSTOM)
...
/obj/item/proc/build_worn_icon(
	...
	override_file = null,
	mutant_styles = NONE,
) // EffigyEdit Change - Further outfit modification for outfits (added `mutant_styles` argument)
