...
/mob/living/carbon/human/update_worn_undersuit()
	...
	if(istype(w_uniform, /obj/item/clothing/under))
		...
		if((dna?.species.bodytype & BODYTYPE_MONKEY) && (uniform.supports_variations_flags & CLOTHING_MONKEY_VARIATION))
			...
			icon_file = uniform.worn_icon_digi || DIGITIGRADE_UNIFORM_FILE // EFFIGY EDIT CHANGE
		...
		uniform_overlay = uniform.build_worn_icon(
			...
			mutant_styles = mutant_styles, // EFFIGY EDIT ADD (#3 Customization - Ported from Skyrat) Taur-friendly uniforms!
		)

		if(!handled_by_bodytype && (OFFSET_UNIFORM in dna.species.offset_features)) // EFFIGY EDIT CHANGE
...
/mob/living/carbon/human/update_worn_gloves()
	...
	if(gloves)
		...
		var/mutable_appearance/gloves_overlay = gloves.build_worn_icon(default_layer = GLOVES_LAYER, default_icon_file = icon_file, override_file = mutant_override ? icon_file : null) // EFFIGY EDIT CHANGE

		if(!mutant_override && (OFFSET_GLOVES in dna.species.offset_features)) // EFFIGY EDIT CHANGE
...
/mob/living/carbon/human/update_worn_glasses()
	...
	if(glasses)
		...
		if(!mutant_override && (OFFSET_GLASSES in dna.species.offset_features)) // EFFIGY EDIT CHANGE
...
/mob/living/carbon/human/update_inv_ears()
	...
	if(ears)
		...
		if(!mutant_override && (OFFSET_EARS in dna.species.offset_features)) // EFFIGY EDIT CHANGE
...
/mob/living/carbon/human/update_worn_neck()
	...
	if(wear_neck)
		...
		if(!mutant_override && (OFFSET_NECK in dna.species.offset_features)) // EFFIGY EDIT CHANGE
...
/mob/living/carbon/human/update_worn_shoes()
	...
	if(shoes)
		...
		var/mutable_appearance/shoes_overlay = shoes.build_worn_icon(default_layer = SHOES_LAYER, default_icon_file = icon_file, override_file = mutant_override ? icon_file : null) // EFFIGY EDIT CHANGE

		if(!mutant_override && (OFFSET_SHOES in dna.species.offset_features)) // EFFIGY EDIT CHANGE
...
/mob/living/carbon/human/update_worn_head()
	...
	if(head)
		...
		if(!mutant_override && (OFFSET_HEAD in dna.species.offset_features)) // EFFIGY EDIT CHANGE
...
/mob/living/carbon/human/update_worn_belt()
	...
	if(belt)
		...
		if(!mutant_override && (OFFSET_BELT in dna.species.offset_features)) // EFFIGY EDIT CHANGE
...
/mob/living/carbon/human/update_worn_oversuit()
	...
	if(wear_suit)
		...
		if(!mutant_override && (OFFSET_UNIFORM in dna.species.offset_features)) // EFFIGY EDIT CHANGE
...
/mob/living/carbon/human/update_worn_mask()
	...
	if(wear_mask)
		...
		if(!mutant_override && (OFFSET_FACEMASK in dna.species.offset_features)) // EFFIGY EDIT CHANGE
...
/mob/living/carbon/human/update_worn_back()
	...
	if(back)
		...
		back_overlay = back.build_worn_icon(default_layer = BACK_LAYER, default_icon_file = icon_file, override_file = mutant_override ? icon_file : null) // EFFIGY EDIT CHANGE

		if(!back_overlay)
			return
		if(!mutant_override && (OFFSET_BACK in dna.species.offset_features)) // EFFIGY EDIT CHANGE
...
/proc/wear_female_version(t_color, icon, layer, type, greyscale_colors)
	...
	return mutable_appearance(GLOB.female_clothing_icons[index], layer = -layer, icon_state = t_color) // EFFIGY EDIT CHANGE - Taur-friendly uniforms and suits - Adds `icon_state = t_color`
...
mutant_styles: The mutant style - taur bodytype, STYLE_TESHARI, etc. // EFFIGY EDIT ADDITION - Taur-friendly suits and uniforms
*/
/obj/item/proc/build_worn_icon(
	...
	override_file = null,
	mutant_styles = NONE,
) // EFFIGY EDIT CHANGE - Further outfit modification for outfits (added `mutant_styles` argument)
	...
	var/list/worn_overlays = worn_overlays(standing, isinhands, file2use, mutant_styles) // EFFIGY EDIT CHANGE - ORIGINAL: var/list/worn_overlays = worn_overlays(standing, isinhands)
