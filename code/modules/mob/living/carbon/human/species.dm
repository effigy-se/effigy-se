...
/datum/species
	...
	var/list/offset_features = list(
		...
		OFFSET_ACCESSORY = list(0, 0), // EFFIGY EDIT ADD - CUSTOMIZATION
	)
	...
	var/list/list/mutant_bodyparts = list() // EFFIGY EDIT CHANGE - CUSTOMIZATION (typed list)
 ...
 */
//EFFIGY EDIT REMOVAL START - CUSTOMIZATION (moved to modular)
/*
		...
		if(ITEM_SLOT_FEET)
			...
			if((bodytype & BODYTYPE_DIGITIGRADE) && !(I.item_flags & IGNORE_DIGITIGRADE))
				if(!(I.supports_variations_flags & (CLOTHING_DIGITIGRADE_VARIATION|CLOTHING_DIGITIGRADE_VARIATION_NO_NEW_ICON)))
					...
					return FALSE
			*/
			// EFFIGY EDIT REMOVE END - CUSTOMIZATION
			return equip_delay_self_check(I, H, bypass_equip_delay_self)
		if(ITEM_SLOT_BELT)
...
/datum/species/proc/harm(mob/living/carbon/human/user, mob/living/carbon/human/target, datum/martial_art/attacker_style)
	...
	if(attacker_style?.harm_act(user,target) == MARTIAL_ATTACK_SUCCESS)
		...
		playsound(target.loc, attacking_bodypart.unarmed_attack_sound || get_sfx("punch"), 25, TRUE, -1) // EFFIGY EDIT CHANGE - ORIGINAL: playsound(target.loc, attacking_bodypart.unarmed_attack_sound, 25, TRUE, -1)
		...
		if(atk_effect == ATTACK_EFFECT_KICK)//kicks deal 1.5x raw damage
			...
			target.apply_damage(damage * PUNCH_STAMINA_MULTIPLIER, STAMINA, affecting, armor_block) // EFFIGY EDIT ADD
			target.apply_damage(damage, attack_type, affecting, armor_block, attack_direction = attack_direction)
		else//other attacks deal full raw damage + 1.5x in stamina damage
			target.apply_damage(damage, attack_type, affecting, armor_block, attack_direction = attack_direction)
			target.apply_damage(damage * PUNCH_STAMINA_MULTIPLIER, STAMINA, affecting, armor_block) // EFFIGY EDIT CHANGE - ORIGINAL: target.apply_damage(damage*1.5, STAMINA, affecting, armor_block)
			if(damage >= 9)
				target.force_say()
	...
	switch(hit_area)
		if(BODY_ZONE_HEAD)
			if(!weapon.get_sharpness() && armor_block < 50)
				...
				if(human.mind && human.stat == CONSCIOUS && human != user && prob(weapon.force + ((MAX_HUMAN_LIFE - human.health) * 0.5))) // rev deconversion through blunt trauma. // EFFIGY EDIT CHANGE
...
/datum/species/proc/apply_damage(damage, damagetype = BRUTE, def_zone = null, blocked, mob/living/carbon/human/H, forced = FALSE, spread_damage = FALSE, wound_bonus = 0, bare_wound_bonus = 0, sharpness = NONE, attack_direction = null)
	...
	switch(damagetype)
		if(BRUTE)
			...
			INVOKE_ASYNC(H, TYPE_PROC_REF(/mob/living/carbon/human, adjust_pain), damage_amount) // EFFIGY EDIT ADD - ERP Pain
		if(BURN)
			H.damageoverlaytemp = 20
...
/datum/species/proc/create_pref_language_perk()
	...
	if(length(bonus_languages))
		to_add += list(list(
			...
			/* EFFIGY EDIT CHANGE START - Digitigrade customization - ORIGINAL:
...
/datum/species/proc/replace_body(mob/living/carbon/target, datum/species/new_species)
	...
	// EFFIGY EDIT ADD START - Synth digitigrade sanitization
