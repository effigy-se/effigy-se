...
/datum/camerachunk/New(x, y, lower_z)
	...
	for(var/z_level in lower_z to upper_z)
		...
		// EFFIGY EDIT - Solarlight Cameras //
		for(var/obj/machinery/solarlight/potential_camera_solarlight in urange(CHUNK_SIZE, locate(x + (CHUNK_SIZE / 2), y + (CHUNK_SIZE / 2), z_level)))
			if(potential_camera_solarlight.solar_camera?.can_use())
				local_cameras += potential_camera_solarlight.solar_camera
		/// EFFIGY EDIT END //
