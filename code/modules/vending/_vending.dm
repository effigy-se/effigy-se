	...
	if(onstation)
		...
		if(!C)
			...
			if(!(usr in GLOB.narcd_underages))
				...
				sec_radio.talk_into(src, "SECURITY ALERT: [usr] recorded attempting to purchase [R.name] in [get_area(src)]. Please watch for substance abuse.", FREQ_SECURITY) // EffigyEdit Change
