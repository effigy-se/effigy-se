...
/obj/item/organ/internal/eyes/proc/generate_body_overlay(mob/living/carbon/human/parent)
	...
	if(isnull(eye_icon_state))
		return list()
	var/eye_icon = parent.dna?.species.eyes_icon || 'icons/mob/species/human/human_face.dmi' // EffigyEdit Add

	var/mutable_appearance/eye_left = mutable_appearance(eye_icon, "[eye_icon_state]_l", -eyes_layer) // EffigyEdit Change - Customization - ORIGINAL: var/mutable_appearance/eye_left = mutable_appearance('icons/mob/human_face.dmi', "[eye_icon_state]_l", -BODY_LAYER)
	var/mutable_appearance/eye_right = mutable_appearance(eye_icon, "[eye_icon_state]_r", -eyes_layer) // EffigyEdit Change - Customization - ORIGINAL: var/mutable_appearance/eye_right = mutable_appearance('icons/mob/human_face.dmi', "[eye_icon_state]_r", -BODY_LAYER)
	...
	// EffigyEdit Add -  (Customization) (darn synths I swear)
	if(eye_icon_state == "None")
		eye_left.alpha = 0
		eye_right.alpha = 0
	// EffigyEdit Add STENDART (Customization) (darn synths I swear)
