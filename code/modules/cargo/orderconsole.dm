...
/obj/machinery/computer/cargo/proc/add_item(params)
	...
	for(var/count in 1 to amount)
		...
		var/datum/supply_order/SO = new(pack = pack ,orderer = name, orderer_rank = rank, orderer_ckey = ckey, reason = reason, paying_account = account, coupon = applied_coupon, charge_on_purchase = TRUE) // EffigyEdit Change
		if(requestonly && !self_paid)
