...
#define UPLOAD_LIMIT_ADMIN 7781440 //Restricts admin client uploads to the server to 2.5MB
...
// EFFIGY EDIT REMOVE END
	...
	if(!client_is_in_db)
		...
		if (CONFIG_GET(flag/panic_bunker) && !holder && !GLOB.deadmins[ckey] && !(ckey in GLOB.bunker_passthrough))
			...
			to_chat_immediate(src, {"<div class='efchatalert_6'>[EFSPAN_ANNOUNCE_MIN_TITLE("Not Authorized \[2F173/2]")]<br>[EFSPAN_ANNOUNCE_MIN_TEXT("Effigy uses a player whitelist and your account was not found! Please visit <a href=\"https://socialdistrict.effigy.se\">https://socialdistrict.effigy.se</a> for details and to submit an application for the server.")]</div>"})
			qdel(query_client_in_db)
			sleep(3 SECONDS)
