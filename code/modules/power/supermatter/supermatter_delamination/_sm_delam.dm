...
/datum/sm_delam/proc/delam_progress(obj/machinery/power/supermatter_crystal/sm)
	...
	if (sm.damage >= sm.emergency_point && sm.damage_archived < sm.emergency_point)
		...
		send2chat(new /datum/tgs_message_content("[sm] has entered the emergency point! It was first powered by [GLOB.main_engine_start_reason]"), CONFIG_GET(string/channel_announce_delam))
