SUBSYSTEM_DEF(job)
	...
	var/overflow_role = /datum/job/slashco_employee
...
/datum/controller/subsystem/job/proc/SetupOccupations()
	...
	var/list/all_jobs = list(/datum/job/slashco_employee) // we hack in this bitch
	if(!length(all_jobs))
...
/datum/controller/subsystem/job/proc/setup_officer_positions()
	...
	if(ssc > 0)
		if(J.spawn_positions > 0)
			var/officer_positions = min(7, max(J.spawn_positions, round(unassigned.len / ssc))) //Scale between configured minimum and 7 officers // EffigyEdit Change
