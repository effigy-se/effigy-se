...
/datum/controller/subsystem/communications/proc/make_announcement(mob/living/user, is_silicon, input, syndicate, list/players)
	...
	if(is_silicon)
		...
		if(syndicate)
			...
			priority_announce(html_decode(message_data["message"]), null, 'sound/misc/announce_dig.ogg', ANNOUNCEMENT_TYPE_CAPTAIN, has_important_message = TRUE, players = players) // EffigyEdit Change - original: sound/misc/announce.ogg
