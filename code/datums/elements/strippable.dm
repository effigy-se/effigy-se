...
/datum/strippable_item/proc/start_unequip(atom/source, mob/user)
	...
	if(ishuman(source) && !is_silent) // EffigyEdit Add (Silent)
...
/proc/start_unequip_mob(obj/item/item, mob/source, mob/user, strip_delay, hidden = FALSE)
	if (!do_after(user, strip_delay || (strip_delay || item.strip_delay) * (HAS_TRAIT(user, TRAIT_STICKY_FINGERS) ? THIEVING_GLOVES_STRIP_SLOWDOWN : NORMAL_STRIP_SLOWDOWN), source, interaction_key = REF(item), hidden = hidden)) // EffigyEdit Change - add strip delay
		return FALSE
