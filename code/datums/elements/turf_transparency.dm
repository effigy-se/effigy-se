...
/datum/element/turf_z_transparency/proc/update_multi_z(turf/our_turf)
	...
	if(isclosedturf(our_turf)) //Show girders below closed turfs
		var/mutable_appearance/girder_underlay = mutable_appearance(EFFIGY_TURFS_ICON_FILE, "girder", layer = BELOW_CLOSED_TURF_LAYER)
		girder_underlay.appearance_flags = RESET_ALPHA | RESET_COLOR
		our_turf.underlays += girder_underlay
		var/mutable_appearance/plating_underlay = mutable_appearance(EFFIGY_TURFS_ICON_FILE, "plating", layer = LOW_FLOOR_LAYER, offset_spokesman = our_turf, plane = FLOOR_PLANE)
