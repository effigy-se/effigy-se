...
/datum/tgs_event_handler/impl/HandleEvent(event_code, ...)
	switch(event_code)
		...
		if(TGS_EVENT_COMPILE_START)
			...
			to_chat(world, span_boxannouncepurple("TGS: Deployment started, new game version incoming..."))
		...
		if(TGS_EVENT_DEPLOYMENT_COMPLETE)
			...
			to_chat(world, span_boxannouncegreen("TGS: Server updated, changes will be applied on the next round..."))
		...
		if(TGS_EVENT_WATCHDOG_SHUTDOWN)
			to_chat_immediate(world, span_boxannouncered("Server is shutting down!"))
