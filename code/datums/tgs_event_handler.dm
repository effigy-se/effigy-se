...
/datum/tgs_event_handler/impl/HandleEvent(event_code, ...)
	switch(event_code)
		...
		if(TGS_EVENT_COMPILE_START)
			...
			to_chat(world, SPAN_BOX_ALERT(PURPLE, "TGS: Deployment started, new game version incoming..."))
		...
		if(TGS_EVENT_DEPLOYMENT_COMPLETE)
			...
			to_chat(world, SPAN_BOX_ALERT(GREEN, "TGS: Server updated, changes will be applied on the next round..."))
		...
		if(TGS_EVENT_WATCHDOG_SHUTDOWN)
			to_chat_immediate(world, SPAN_BOX_ALERT(RED, "Server is shutting down!"))
