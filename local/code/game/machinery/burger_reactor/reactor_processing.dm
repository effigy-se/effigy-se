/obj/machinery/power/rbmk2/process()
	...
	if(active && rod_mix.temperature > stored_rod.temperature_limit || last_power_generation > max_power_generation*(1.1 + rand()) )
		if(!meltdown)
			...
			meltdown = TRUE
		var/chosen_sound = pick('local/sound/machines/rbmk2/failure01.ogg','local/sound/machines/rbmk2/failure02.ogg','local/sound/machines/rbmk2/failure03.ogg','local/sound/machines/rbmk2/failure04.ogg')
...
/obj/machinery/power/rbmk2/process_atmos() //Only turf air related stuff is handled here.
	...
	if(stored_rod && meltdown)
		...
		playsound(src, 'local/sound/machines/rbmk2/ionization.ogg', 50, TRUE, extrarange = ionize_air_range)
	else
