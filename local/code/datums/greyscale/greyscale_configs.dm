/*
*	HEAD
*/

// BERETS
/datum/greyscale_config/beret
	name = "Beret"
	icon_file = 'local/icons/mob/clothing/greyscale/beret.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/beret/beret.json'

/datum/greyscale_config/beret/worn
	name = "Beret Worn"
	json_config = 'local/code/datums/greyscale/json_configs/beret/beret_worn.json'

/datum/greyscale_config/beret/worn/vox
	name = "Beret Worn - Vox"
	icon_file = 'local/icons/mob/clothing/greyscale/beret_vox.dmi'

/datum/greyscale_config/beret_badge
	name = "Beret With Badge"
	icon_file = 'local/icons/mob/clothing/greyscale/beret.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/beret/beret_badge.json'

/datum/greyscale_config/beret_badge/worn
	name = "Beret With Badge Worn"
	json_config = 'local/code/datums/greyscale/json_configs/beret/beret_badge_worn.json'

/datum/greyscale_config/beret_badge/worn/vox
	name = "Beret With Badge Worn - Vox"
	icon_file = 'local/icons/mob/clothing/greyscale/beret_vox.dmi'

/datum/greyscale_config/beret_badge/worn/teshari
	name = "Beret With Badge Worn - Teshari"
	icon_file = 'local/icons/mob/clothing/greyscale/beret_teshari.dmi'

//COWBOY
/datum/greyscale_config/cowboy_wide
	name = "Wide Brimmed Hat"
	icon_file = 'local/icons/obj/clothing/head/cowboy.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/head/cowboy/two_layer_hats.json'

/datum/greyscale_config/cowboy_wide/worn
	name = "Wide Brimmed Hat (Worn)"
	icon_file = 'local/icons/mob/clothing/head/cowboy.dmi'

/datum/greyscale_config/cowboy_wide_feathered
	name = "Wide Brimmed Feathered Hat"
	icon_file = 'local/icons/obj/clothing/head/cowboy.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/head/cowboy/three_layer_hats.json'

/datum/greyscale_config/cowboy_wide_feathered/worn
	name = "Wide Brimmed Feathered Hat (Worn)"
	icon_file = 'local/icons/mob/clothing/head/cowboy.dmi'

/datum/greyscale_config/cowboy_flat
	name = "Flat Brimmed Hat"
	icon_file = 'local/icons/obj/clothing/head/cowboy.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/head/cowboy/two_layer_hats.json'

/datum/greyscale_config/cowboy_flat/worn
	name = "Flat Brimmed Hat (Worn)"
	icon_file = 'local/icons/mob/clothing/head/cowboy.dmi'

/datum/greyscale_config/cowboy_flat_cowl
	name = "Flat Brimmed Hat with Cowl"
	icon_file = 'local/icons/obj/clothing/head/cowboy.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/head/cowboy/three_layer_hats.json'

/datum/greyscale_config/cowboy_flat_cowl/worn
	name = "Flat Brimmed Hat with Cowl (Worn)"
	icon_file = 'local/icons/mob/clothing/head/cowboy.dmi'

/datum/greyscale_config/cowboy_cattleman
	name = "Cattleman Hat"
	icon_file = 'local/icons/obj/clothing/head/cowboy.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/head/cowboy/two_layer_hats.json'

/datum/greyscale_config/cowboy_cattleman/worn
	name = "Cattleman Hat (Worn)"
	icon_file = 'local/icons/mob/clothing/head/cowboy.dmi'

/datum/greyscale_config/cowboy_cattleman_wide
	name = "Wide Brimmed Cattleman Hat"
	icon_file = 'local/icons/obj/clothing/head/cowboy.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/head/cowboy/two_layer_hats.json'

/datum/greyscale_config/cowboy_cattleman_wide/worn
	name = "Wide Brimmed Cattleman Hat (Worn)"
	icon_file = 'local/icons/mob/clothing/head/cowboy.dmi'

//MUSHROOM CAP

/datum/greyscale_config/mushcap
	name = "Mushroom Cap"
	icon_file = 'local/icons/mob/clothing/greyscale/mushcap.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/mush_cap/mushcap.json'

/datum/greyscale_config/mushcap/worn
	name = "Mushroom Cap Worn"
	json_config = 'local/code/datums/greyscale/json_configs/mush_cap/mushcap_worn.json'

// FLATCAP

/datum/greyscale_config/flatcap
	name = "Flat Cap"
	icon_file = 'local/icons/mob/clothing/greyscale/hats.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/flatcap/flatcap.json'

/datum/greyscale_config/flatcap/worn
	name = "Flat Cap (Worn)"
	icon_file = 'local/icons/mob/clothing/greyscale/hats.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/flatcap/flatcap_worn.json'

// FLOWER PIN

/datum/greyscale_config/flowerpin
	name = "Flower Pin"
	icon_file = 'local/icons/mob/clothing/greyscale/hats.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/flowerpin/flowerpin.json'

/datum/greyscale_config/flowerpin/worn
	name = "Flower Pin (Worn)"
	icon_file = 'local/icons/mob/clothing/greyscale/hats.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/flowerpin/flowerpin_worn.json'

// Hood

/datum/greyscale_config/standalone_hood
	name = "Hood"
	icon_file = 'local/icons/mob/clothing/greyscale/head/head.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/head/hood/hood.json'

/datum/greyscale_config/standalone_hood/worn
	name = "Hood (Worn)"
	json_config = 'local/code/datums/greyscale/json_configs/head/hood/hood_worn.json'

/datum/greyscale_config/standalone_hood/worn/teshari
	icon_file = 'local/icons/mob/clothing/greyscale/head/head_teshari.dmi'

/datum/greyscale_config/standalone_hood/worn/newvox
	icon_file = 'local/icons/mob/clothing/greyscale/head/head_newvox.dmi'

/datum/greyscale_config/standalone_hood/worn/oldvox
	icon_file = 'local/icons/mob/clothing/greyscale/head/head_oldvox.dmi'

/*
*	MASKS
*/

// CLOWN
/datum/greyscale_config/clown_mask
	name = "Colourable Clown Mask"
	icon_file = 'local/icons/mob/clothing/greyscale/clown_mask.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/clown_mask/clown_mask.json'

/datum/greyscale_config/clown_mask/worn
	name = "Colourable Clown Mask (Worn)"
	json_config = 'local/code/datums/greyscale/json_configs/clown_mask/clown_mask_worn.json'

// VAPE
/datum/greyscale_config/vape/worn/digi
	name = "Worn Vape - Digitigrade"
	icon_file = 'local/icons/mob/clothing/mask.dmi'

// RESPIRATOR
/datum/greyscale_config/respirator
	name = "Colourable Respirator"
	icon_file = 'local/icons/mob/clothing/greyscale/masks.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/respirator/respirator.json'

/datum/greyscale_config/respirator/worn
	name = "Colourable Respirator (Worn)"
	json_config = 'local/code/datums/greyscale/json_configs/respirator/respirator_worn.json'

/datum/greyscale_config/respirator/worn/snouted
	name = "Colourable Respirator (Worn) (Snouted)"
	icon_file = 'local/icons/mob/clothing/greyscale/masks_snout.dmi'

/datum/greyscale_config/respirator/worn/better_vox
	name = "Colourable Respirator (Worn) (Vox Primalis)"
	icon_file = 'local/icons/mob/clothing/greyscale/masks_vox_better.dmi'

/datum/greyscale_config/respirator/worn/vox
	name = "Colourable Respirator (Worn) (Vox)"
	icon_file = 'local/icons/mob/clothing/greyscale/masks_vox.dmi'

/datum/greyscale_config/respirator/worn/teshari
	name = "Colourable Respirator (Worn) (Teshari)"
	icon_file = 'local/icons/mob/clothing/greyscale/masks_teshari.dmi'

// STERILE
/datum/greyscale_config/sterile_mask
	name = "Colourable Sterile Mask"
	icon_file = 'local/icons/mob/clothing/greyscale/masks.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/sterile_mask/sterile.json'

/datum/greyscale_config/sterile_mask/worn
	name = "Colourable Sterile Mask (Worn)"
	json_config = 'local/code/datums/greyscale/json_configs/sterile_mask/sterile_worn.json'

/datum/greyscale_config/sterile_mask/worn/snouted
	name = "Colourable Sterile Mask (Worn) (Snouted)"
	icon_file = 'local/icons/mob/clothing/greyscale/masks_snout.dmi'

/datum/greyscale_config/sterile_mask/worn/better_vox
	name = "Colourable Sterile Mask (Worn) (Vox Primalis)"
	icon_file = 'local/icons/mob/clothing/greyscale/masks_vox_better.dmi'

/datum/greyscale_config/sterile_mask/worn/vox
	name = "Colourable Sterile Mask (Worn) (Vox)"
	icon_file = 'local/icons/mob/clothing/greyscale/masks_vox.dmi'

/datum/greyscale_config/sterile_mask/worn/teshari
	name = "Colourable Sterile Mask (Worn) (Teshari)"
	icon_file = 'local/icons/mob/clothing/greyscale/masks_teshari.dmi'

/*
*	NECK
*/

// RANGER PONCHO
/datum/greyscale_config/ranger_poncho
	name = "Ranger Poncho"
	icon_file = 'local/icons/mob/clothing/greyscale/ranger_poncho.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/ranger_poncho/ranger_poncho.json'

/datum/greyscale_config/ranger_poncho/worn
	name = "Ranger Poncho Worn"
	json_config = 'local/code/datums/greyscale/json_configs/ranger_poncho/ranger_poncho_worn.json'

/datum/greyscale_config/ranger_poncho/worn/teshari
	name = "Ranger Poncho Worn - Teshari"
	icon_file = 'local/icons/mob/clothing/greyscale/ranger_poncho_teshari.dmi'

// CLOAKS
/datum/greyscale_config/cloak
	name = "Cloak"
	icon_file = 'local/icons/mob/clothing/greyscale/cloak.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/cloak/cloak.json'

/datum/greyscale_config/cloak/worn
	name = "Cloak Worn"
	json_config = 'local/code/datums/greyscale/json_configs/cloak/cloak_worn.json'

/datum/greyscale_config/cloak/veil
	name = "Veil"
	json_config = 'local/code/datums/greyscale/json_configs/cloak/veil.json'

/datum/greyscale_config/cloak/veil/worn
	name = "Veil Worn"
	json_config = 'local/code/datums/greyscale/json_configs/cloak/veil_worn.json'

/datum/greyscale_config/cloak/boat
	name = "Boatcloak"
	json_config = 'local/code/datums/greyscale/json_configs/cloak/boat.json'

/datum/greyscale_config/cloak/boat/worn
	name = "Boatcloak Worn"
	json_config = 'local/code/datums/greyscale/json_configs/cloak/boat_worn.json'

/datum/greyscale_config/cloak/shroud
	name = "Shroud"
	json_config = 'local/code/datums/greyscale/json_configs/cloak/shroud.json'

/datum/greyscale_config/cloak/shroud/worn
	name = "Shroud Worn"
	json_config = 'local/code/datums/greyscale/json_configs/cloak/shroud_worn.json'

// Mantle

/datum/greyscale_config/mantle
	name = "Mantle"
	icon_file = 'local/icons/mob/clothing/greyscale/neck/neck.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/neck/mantle/mantle.json'

/datum/greyscale_config/mantle/worn
	name = "Mantle (Worn)"
	json_config = 'local/code/datums/greyscale/json_configs/neck/mantle/mantle_worn.json'

/datum/greyscale_config/mantle/worn/teshari
	icon_file = 'local/icons/mob/clothing/greyscale/neck/neck_teshari.dmi'

/datum/greyscale_config/mantle/worn/newvox
	icon_file = 'local/icons/mob/clothing/greyscale/neck/neck_newvox.dmi'

/datum/greyscale_config/mantle/worn/oldvox
	icon_file = 'local/icons/mob/clothing/greyscale/neck/neck_oldvox.dmi'

// COLLARS

/datum/greyscale_config/collar
	name = "Collar"
	icon_file = 'local/icons/mob/clothing/greyscale/collar.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/collar/bell.json'

// Bell
/datum/greyscale_config/collar/bell
	name = "Bell Collar"
	json_config = 'local/code/datums/greyscale/json_configs/collar/bell.json'

/datum/greyscale_config/collar/bell/worn
	name = "Bell Collar (Worn)"
	json_config = 'local/code/datums/greyscale/json_configs/collar/bell_worn.json'

// Choker
/datum/greyscale_config/collar/choker
	name = "Choker"
	json_config = 'local/code/datums/greyscale/json_configs/collar/choker.json'

/datum/greyscale_config/collar/choker/worn
	name = "Choker (Worn)"
	json_config = 'local/code/datums/greyscale/json_configs/collar/choker_worn.json'

// Cow
/datum/greyscale_config/collar/cow
	name = "Cowbell Collar"
	json_config = 'local/code/datums/greyscale/json_configs/collar/cow.json'

/datum/greyscale_config/collar/cow/worn
	name = "Cowbell Collar (Worn)"
	json_config = 'local/code/datums/greyscale/json_configs/collar/cow_worn.json'

// Cross
/datum/greyscale_config/collar/cross
	name = "Cross Collar"
	json_config = 'local/code/datums/greyscale/json_configs/collar/cross.json'

/datum/greyscale_config/collar/cross/worn
	name = "Cross Collar (Worn)"
	json_config = 'local/code/datums/greyscale/json_configs/collar/cross_worn.json'

// Holo
/datum/greyscale_config/collar/holo
	name = "Holo Collar"
	json_config = 'local/code/datums/greyscale/json_configs/collar/holo.json'

/datum/greyscale_config/collar/holo/worn
	name = "Holo Collar (Worn)"
	json_config = 'local/code/datums/greyscale/json_configs/collar/holo_worn.json'

// Leather
/datum/greyscale_config/collar/leather
	name = "Leather Collar"
	json_config = 'local/code/datums/greyscale/json_configs/collar/leather.json'

/datum/greyscale_config/collar/leather/worn
	name = "Leather Collar (Worn)"
	json_config = 'local/code/datums/greyscale/json_configs/collar/leather_worn.json'

// Pet
/datum/greyscale_config/collar/pet
	name = "Pet Collar"
	json_config = 'local/code/datums/greyscale/json_configs/collar/pet.json'

/datum/greyscale_config/collar/pet/worn
	name = "Pet Collar (Worn)"
	json_config = 'local/code/datums/greyscale/json_configs/collar/pet_worn.json'

// Spiked
/datum/greyscale_config/collar/spike
	name = "Spiked Collar"
	json_config = 'local/code/datums/greyscale/json_configs/collar/spike.json'

/datum/greyscale_config/collar/spike/worn
	name = "Spiked Collar (Worn)"
	json_config = 'local/code/datums/greyscale/json_configs/collar/spike_worn.json'


/*
*	SUITS
*/

// Apron

/datum/greyscale_config/apron
	name = "Apron"
	icon_file = 'local/icons/mob/clothing/greyscale/suit/suit.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/suits/apron/apron.json'

/datum/greyscale_config/apron/worn
	name = "Apron (Worn)"
	json_config = 'local/code/datums/greyscale/json_configs/suits/apron/apron_worn.json'

/datum/greyscale_config/apron/worn/teshari
	icon_file = 'local/icons/mob/clothing/greyscale/suit/suit_teshari.dmi'

/datum/greyscale_config/apron/worn/newvox
	icon_file = 'local/icons/mob/clothing/greyscale/suit/suit_newvox.dmi'

/datum/greyscale_config/apron/worn/oldvox
	icon_file = 'local/icons/mob/clothing/greyscale/suit/suit_oldvox.dmi'

// ARMOR

/datum/greyscale_config/heck_suit/worn/digi
	icon_file = 'local/icons/mob/clothing/suit_digi.dmi'

//Helmet pairs with the above suit
/datum/greyscale_config/heck_helmet/worn/snouted
	icon_file = 'local/icons/mob/clothing/head/helmet_muzzled.dmi'

// Bathrobes

/datum/greyscale_config/bathrobe
	name = "Bathrobe"
	icon_file = 'local/icons/mob/clothing/greyscale/suit/suit.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/suits/bathrobe/bathrobe.json'

/datum/greyscale_config/bathrobe/worn
	name = "Worn Bathrobe"
	json_config = 'local/code/datums/greyscale/json_configs/suits/bathrobe/bathrobe_worn.json'

/datum/greyscale_config/bathrobe/worn/teshari
	icon_file = 'local/icons/mob/clothing/greyscale/suit/suit_teshari.dmi'

/datum/greyscale_config/bathrobe/worn/newvox
	icon_file = 'local/icons/mob/clothing/greyscale/suit/suit_newvox.dmi'

/datum/greyscale_config/bathrobe/worn/oldvox
	icon_file = 'local/icons/mob/clothing/greyscale/suit/suit_oldvox.dmi'

// FANCY SUITS
/datum/greyscale_config/fancy_suit
	name = "Fancy Suit"
	icon_file = 'local/icons/mob/clothing/greyscale/fancy_suits.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/fancy_suits/fancy_suit.json'

/datum/greyscale_config/fancy_suit/worn
	name = "Worn Fancy Suit"
	json_config = 'local/code/datums/greyscale/json_configs/fancy_suits/fancy_suit_worn.json'

/datum/greyscale_config/recolorable_suit
	name = "Formal Suit"
	icon_file = 'local/icons/obj/clothing/under/suits.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/fancy_suits/recolorable_suit.json'

/datum/greyscale_config/recolorable_suit/worn
	name = "Worn Formal Suit"
	icon_file = 'local/icons/mob/clothing/under/suits.dmi'

/datum/greyscale_config/recolorable_suit/worn/digi
	name = "Worn Formal Suit - Digitigrade"
	icon_file = 'local/icons/mob/clothing/under/suits_digi.dmi'

/datum/greyscale_config/recolorable_suitskirt
	name = "Formal Suitskirt"
	icon_file = 'local/icons/obj/clothing/under/suits.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/fancy_suits/recolorable_suit.json'

/datum/greyscale_config/recolorable_suitskirt/worn
	name = "Worn Formal Suitskirt"
	icon_file = 'local/icons/mob/clothing/under/suits.dmi'

/datum/greyscale_config/recolorable_suitskirt/worn/digi
	name = "Worn Formal Suitskirt - Digitigrade"
	icon_file = 'local/icons/mob/clothing/under/suits_digi.dmi'

// HOODIES
/datum/greyscale_config/hoodie
	name = "Hoodie"
	icon_file = 'local/icons/mob/clothing/greyscale/hoodie.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/hoodie/hoodie.json'

/datum/greyscale_config/hoodie/worn
	name = "Hoodie Worn"
	json_config = 'local/code/datums/greyscale/json_configs/hoodie/hoodie_worn.json'

/datum/greyscale_config/hoodie_trim
	name = "Trimmed Hoodie"
	icon_file = 'local/icons/mob/clothing/greyscale/hoodie.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/hoodie/hoodie_trim.json'

/datum/greyscale_config/hoodie_trim/worn
	name = "Trimmed Hoodie Worn"
	json_config = 'local/code/datums/greyscale/json_configs/hoodie/hoodie_trim_worn.json'

/datum/greyscale_config/hoodie_branded
	name = "Branded Hoodie"
	icon_file = 'local/icons/mob/clothing/greyscale/hoodie.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/hoodie/hoodie_branded.json'

/datum/greyscale_config/hoodie_branded/worn
	name = "Branded Hoodie Worn"
	json_config = 'local/code/datums/greyscale/json_configs/hoodie/hoodie_branded_worn.json'

// CARDIGAN

/datum/greyscale_config/cardigan
	name = "Cardigan"
	icon_file = 'local/icons/mob/clothing/greyscale/suit.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/cardigan/cardigan.json'

/datum/greyscale_config/cardigan/worn
	name = "Cardigan (Worn)"
	json_config = 'local/code/datums/greyscale/json_configs/cardigan/cardigan_worn.json'

// SWEATERS

/datum/greyscale_config/cableknit_sweater
	name = "Cableknit Sweater"
	icon_file = 'local/icons/mob/clothing/greyscale/sweaters.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/sweater/sweater.json'

/datum/greyscale_config/cableknit_sweater/worn
	name = "Cableknit Sweater (Worn)"
	json_config = 'local/code/datums/greyscale/json_configs/sweater/sweater_worn.json'

//Keyhole uses the same configs!

// OVERALLS

/datum/greyscale_config/overalls
	name = "Overalls"
	icon_file = 'local/icons/mob/clothing/greyscale/suit/suit.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/overalls/overalls.json'

/datum/greyscale_config/overalls/worn
	name = "Worn Overalls"
	json_config = 'local/code/datums/greyscale/json_configs/overalls/overalls_worn.json'

/datum/greyscale_config/overalls/worn/digi
	name = "Worn Overalls - Digitigrade"
	icon_file = 'local/icons/mob/clothing/greyscale/suit/suit_digi.dmi'

// URBAN COAT

/datum/greyscale_config/urban_coat
	name = "Urban Coat"
	icon_file = 'local/icons/mob/clothing/greyscale/urban_coat.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/urban_coat/urban_coat.json'

/datum/greyscale_config/urban_coat/worn
	name = "Urban Coat (Worn)"
	json_config = 'local/code/datums/greyscale/json_configs/urban_coat/urban_coat_worn.json'

// LEATHER JACKET

/datum/greyscale_config/leather_jacket
	name = "Leather Jacket"
	icon_file = 'local/icons/mob/clothing/greyscale/leather_jacket.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/leather_jacket/leather_jacket.json'

/datum/greyscale_config/leather_jacket/worn
	name = "Leather Jacket (Worn)"
	json_config = 'local/code/datums/greyscale/json_configs/leather_jacket/leather_jacket_worn.json'

// DUSTER

/datum/greyscale_config/duster
	name = "Duster"
	icon_file = 'local/icons/mob/clothing/greyscale/duster.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/duster/duster.json'

/datum/greyscale_config/duster/worn
	name = "Duster (Worn)"
	json_config = 'local/code/datums/greyscale/json_configs/duster/duster_worn.json'

// PEACOAT

/datum/greyscale_config/peacoat
	name = "Peacoat"
	icon_file = 'local/icons/mob/clothing/greyscale/peacoat.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/peacoat/peacoat.json'

/datum/greyscale_config/peacoat/worn
	name = "Peacoat (Worn)"
	json_config = 'local/code/datums/greyscale/json_configs/peacoat/peacoat_worn.json'

// VARSITY JACKET

/datum/greyscale_config/varsity
	name = "Varsity Jacket"
	icon_file = 'local/icons/mob/clothing/greyscale/suit/suit.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/suits/varsity/varsity.json'

/datum/greyscale_config/varsity/worn
	name = "Varsity Jacket (Worn)"
	json_config = 'local/code/datums/greyscale/json_configs/suits/varsity/varsity_worn.json'

// TAILORED JACKET

/datum/greyscale_config/tailored_jacket
	name = "Tailored Jacket"
	icon_file = 'local/icons/mob/clothing/greyscale/suit/suit.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/suits/tailored_jacket/tailored_jacket.json'

/datum/greyscale_config/tailored_jacket/worn
	name = "Tailored Jacket (Worn)"
	json_config = 'local/code/datums/greyscale/json_configs/suits/tailored_jacket/tailored_jacket_worn.json'

/datum/greyscale_config/tailored_short_jacket
	name = "Tailored Short Jacket"
	icon_file = 'local/icons/mob/clothing/greyscale/suit/suit.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/suits/tailored_short_jacket/tailored_short_jacket.json'

/datum/greyscale_config/tailored_short_jacket/worn
	name = "Tailored Short Jacket (Worn)"
	json_config = 'local/code/datums/greyscale/json_configs/suits/tailored_short_jacket/tailored_short_jacket_worn.json'


/*
*	UNDER
*/


// JUMPSUITS

/datum/greyscale_config/jumpsuit/worn/digi
	name = "Worn Jumpsuit - Digitigrade"
	icon_file = 'local/icons/mob/clothing/under/color_digi.dmi'

/datum/greyscale_config/jumpsuit/prison/worn/digi
	name = "Worn Prison Jumpsuit - Digitigrade"
	icon_file = 'local/icons/mob/clothing/under/color_digi.dmi'

/datum/greyscale_config/jumpsuit/worn/monkey
	name = "Worn Jumpsuit - Monkey"
	icon_file = MONKEY_UNIFORM_FILE
	json_config = 'local/code/datums/greyscale/json_configs/jumpsuit/jumpsuit_worn_monkey.json'

/datum/greyscale_config/jumpsuit/prison/worn/monkey
	name = "Worn Prison Jumpsuit - Monkey"
	icon_file = MONKEY_UNIFORM_FILE
	json_config = 'local/code/datums/greyscale/json_configs/jumpsuit/jumpsuit_prison_worn_monkey.json'

/datum/greyscale_config/jumpsuit/worn/vox
	name = "Worn Jumpsuit - Vox"
	icon_file = 'local/icons/mob/clothing/greyscale/jumpsuit_vox.dmi'

/datum/greyscale_config/jumpsuit/prison/worn/vox
	name = "Worn Prison Jumpsuit - Vox"
	icon_file = 'local/icons/mob/clothing/greyscale/jumpsuit_vox.dmi'

/datum/greyscale_config/jumpsuit/worn/better_vox
	name = "Worn Jumpsuit - Better Vox"
	icon_file = 'local/icons/mob/clothing/greyscale/jumpsuit_better_vox.dmi'

/datum/greyscale_config/jumpsuit/prison/worn/better_vox
	name = "Worn Prison Jumpsuit - Better Vox"
	icon_file = 'local/icons/mob/clothing/greyscale/jumpsuit_better_vox.dmi'

/datum/greyscale_config/jumpsuit/worn/taur/snake
	name = "Worn Jumpsuit - Naga"
	icon_file = 'local/icons/mob/clothing/greyscale/jumpsuit_snake.dmi'

/datum/greyscale_config/jumpsuit/prison/worn/taur/snake
	name = "Worn Prison Jumpsuit - Naga"
	icon_file = 'local/icons/mob/clothing/greyscale/jumpsuit_snake.dmi'

/datum/greyscale_config/jumpsuit/worn/teshari
	name = "Worn Jumpsuit - Teshari"
	icon_file = 'local/icons/mob/clothing/greyscale/jumpsuit_teshari.dmi'

/datum/greyscale_config/jumpsuit/prison/worn/teshari
	name = "Worn Prison Jumpsuit - Teshari"
	icon_file = 'local/icons/mob/clothing/greyscale/jumpsuit_teshari.dmi'

// PANTS / SHORTS
//Note, these icons are now in master_files per TG's sorting. There's no reason to NOT have these pieces in master files now that they're sorted out.
//The rest will eventually join them - preferably with a cleaner method too instead of having to have unique configs for every single item's digi state.

/datum/greyscale_config/slacks/worn/digi
	name = "Worn Slacks - Digitigrade"
	icon_file = SHORTS_PANTS_SHIRTS_DIGIFILE

/datum/greyscale_config/jeans/worn/digi
	name = "Worn Jeans - Digitigrade"
	icon_file = SHORTS_PANTS_SHIRTS_DIGIFILE

/datum/greyscale_config/shorts/worn/digi
	name = "Worn Shorts - Digitigrade"
	icon_file = SHORTS_PANTS_SHIRTS_DIGIFILE

/datum/greyscale_config/jeanshorts/worn/digi
	name = "Worn Jean Shorts - Digitigrade"
	icon_file = SHORTS_PANTS_SHIRTS_DIGIFILE

/datum/greyscale_config/shorts_ripped
	name = "Ripped Shorts"
	icon_file = 'local/icons/obj/clothing/under/shorts_pants_shirts.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/pants_shorts_skirts_dresses/shorts_ripped.json'

/datum/greyscale_config/shorts_ripped/worn //TG will hopefully start having worn as a subtype, it means only needing 1 .json
	name = "Worn Ripped Shorts"
	icon_file = 'local/icons/mob/clothing/under/shorts_pants_shirts.dmi'

/datum/greyscale_config/shorts_ripped/worn/digi
	name = "Worn Ripped Shorts - Digitigrade"
	icon_file = SHORTS_PANTS_SHIRTS_DIGIFILE

/datum/greyscale_config/shorts_ripped/worn/teshari
	name = "Worn Ripped Shorts - Teshari"
	icon_file = 'local/icons/mob/clothing/greyscale/teshari_uniform.dmi'

/datum/greyscale_config/jeans_ripped
	name = "Ripped Jeans"
	icon_file = 'local/icons/obj/clothing/under/shorts_pants_shirts.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/pants_shorts_skirts_dresses/jeans_ripped.json'

/datum/greyscale_config/jeans_ripped/worn
	name = "Worn Ripped Jeans"
	icon_file = 'local/icons/mob/clothing/under/shorts_pants_shirts.dmi'

/datum/greyscale_config/jeans_ripped/worn/digi
	name = "Worn Ripped Jeans - Digitigrade"
	icon_file = SHORTS_PANTS_SHIRTS_DIGIFILE

/datum/greyscale_config/yoga_pants
	name = "Yoga Pants"
	icon_file = 'local/icons/obj/clothing/under/shorts_pants_shirts.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/pants_shorts_skirts_dresses/yoga_pants.json'

/datum/greyscale_config/yoga_pants/worn
	name = "Worn Yoga Pants"
	icon_file = 'local/icons/mob/clothing/under/shorts_pants_shirts.dmi'

/datum/greyscale_config/yoga_pants/worn/digi
	name = "Worn Yoga Pants - Digitigrade"
	icon_file = SHORTS_PANTS_SHIRTS_DIGIFILE

// DRESSES / SKIRTS

/datum/greyscale_config/plaidskirt/worn/digi
	name = "Worn Plaid Skirt - Digitigrade"
	icon_file = SKIRTS_DRESSES_DIGIFILE

/datum/greyscale_config/lone_skirt
	name = "Skirt"
	icon_file = 'local/icons/obj/clothing/under/skirts_dresses.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/pants_shorts_skirts_dresses/lone_skirt.json'

/datum/greyscale_config/lone_skirt/worn
	name = "Worn Skirt"
	icon_file = 'local/icons/mob/clothing/under/skirts_dresses.dmi'

/datum/greyscale_config/turtleskirt_knit
	name = "Cableknit Skirtleneck"
	icon_file = 'local/icons/obj/clothing/under/skirts_dresses.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/pants_shorts_skirts_dresses/turtleskirt_knit.json'

/datum/greyscale_config/turtleskirt_knit/worn
	name = "Worn Cableknit Skirtleneck"
	icon_file = 'local/icons/mob/clothing/under/skirts_dresses.dmi'

/datum/greyscale_config/jean_skirt
	name = "Jean Skirt"
	icon_file = 'local/icons/obj/clothing/under/skirts_dresses.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/pants_shorts_skirts_dresses/jean_skirt.json'

/datum/greyscale_config/jean_skirt/worn
	name = "Worn Jean Skirt"
	icon_file = 'local/icons/mob/clothing/under/skirts_dresses.dmi'

/datum/greyscale_config/jean_skirt/worn/digi
	name = "Worn Jean Skirt - Digitigrade"
	icon_file = SKIRTS_DRESSES_DIGIFILE

/datum/greyscale_config/short_dress
	name = "Short Dress"
	icon_file = 'local/icons/obj/clothing/under/skirts_dresses.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/pants_shorts_skirts_dresses/short_dress.json'

/datum/greyscale_config/short_dress/worn
	name = "Worn Short Dress"
	icon_file = 'local/icons/mob/clothing/under/skirts_dresses.dmi'

/datum/greyscale_config/strapless_dress
	name = "Strapless Dress"
	icon_file = 'local/icons/obj/clothing/under/skirts_dresses.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/pants_shorts_skirts_dresses/strapless_dress.json'

/datum/greyscale_config/strapless_dress/worn
	name = "Worn Strapless Dress"
	icon_file = 'local/icons/mob/clothing/under/skirts_dresses.dmi'

/datum/greyscale_config/pentagram_dress
	name = "Pentagram Strapped Dress"
	icon_file = 'local/icons/obj/clothing/under/skirts_dresses.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/pants_shorts_skirts_dresses/pentagram_dress.json'

/datum/greyscale_config/pentagram_dress/worn
	name = "Worn Pentagram Strapped Dress"
	icon_file = 'local/icons/mob/clothing/under/skirts_dresses.dmi'

/datum/greyscale_config/plaidskirt/worn/teshari
	name = "Worn Plaid Skirt - Teshari"
	icon_file = 'local/icons/mob/clothing/greyscale/dressteshari.dmi'

/datum/greyscale_config/sundress/worn/teshari
	name = "Worn Sundress - Teshari"
	icon_file = 'local/icons/mob/clothing/greyscale/dressteshari.dmi'

// KILT
// COSTUMES

/datum/greyscale_config/buttondown_slacks/worn/digi
	name = "Worn Buttondown with Slacks - Digitigrade"
	icon_file = SHORTS_PANTS_SHIRTS_DIGIFILE

/datum/greyscale_config/buttondown_shorts/worn/digi
	name = "Worn Buttondown with Shorts - Digitigrade"
	icon_file = SHORTS_PANTS_SHIRTS_DIGIFILE

/datum/greyscale_config/football_suit/worn/digi
	icon_file = 'local/icons/mob/clothing/under/costume_digi.dmi'

/datum/greyscale_config/qipao
	name = "Qipao"
	icon_file = 'local/icons/obj/clothing/under/costume.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/costumes/lunar_japanese.json'

/datum/greyscale_config/qipao/worn
	name = "Worn Qipao"
	icon_file = 'local/icons/mob/clothing/under/costume.dmi'

/datum/greyscale_config/qipao/worn/digi
	name = "Worn Qipao - Digitigrade"
	icon_file = 'local/icons/mob/clothing/under/costume_digi.dmi'

/datum/greyscale_config/cheongsam
	name = "Cheongsam"
	icon_file = 'local/icons/obj/clothing/under/costume.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/costumes/lunar_japanese.json'

/datum/greyscale_config/cheongsam/worn
	name = "Worn Cheongsam"
	icon_file = 'local/icons/mob/clothing/under/costume.dmi'

/datum/greyscale_config/cheongsam/worn/digi
	name = "Worn Cheongsam - Digitigrade"
	icon_file = 'local/icons/mob/clothing/under/costume_digi.dmi'

/datum/greyscale_config/yukata
	name = "Yukata"
	icon_file = 'local/icons/obj/clothing/under/costume.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/costumes/lunar_japanese.json'

/datum/greyscale_config/yukata/worn
	name = "Worn Yukata"
	icon_file = 'local/icons/mob/clothing/under/costume.dmi'

/datum/greyscale_config/yukata/worn/digi
	name = "Worn Yukata - Digitigrade"
	icon_file = 'local/icons/mob/clothing/under/costume_digi.dmi'

/datum/greyscale_config/kilt
	name = "Kilt"
	icon_file = 'local/icons/obj/clothing/under/shorts_pants_shirts.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/kilt/kilt.json'

/datum/greyscale_config/kilt/worn
	name = "Worn Kilt"
	icon_file = 'local/icons/mob/clothing/under/shorts_pants_shirts.dmi'

/datum/greyscale_config/kilt/worn/digi
	name = "Worn Kilt - Digitigrade"
	icon_file = SHORTS_PANTS_SHIRTS_DIGIFILE

/datum/greyscale_config/buttondown_vicvest
	name = "Buttondown with Double-breasted Vest"
	icon_file = 'local/icons/obj/clothing/under/shorts_pants_shirts.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/costumes/buttondown_vicvest.json'

/datum/greyscale_config/buttondown_vicvest/worn
	name = "Worn Buttondown with Double-breasted Vest"
	icon_file = 'local/icons/mob/clothing/under/shorts_pants_shirts.dmi'

/datum/greyscale_config/buttondown_vicvest/worn/digi
	name = "Worn Buttondown with Double-breasted Vest - Digitigrade"
	icon_file = SHORTS_PANTS_SHIRTS_DIGIFILE

/*
TREK
*/

/datum/greyscale_config/trek/worn/digi
	name = "Worn Trek Uniform - Digitigrade"
	icon_file = 'local/icons/mob/clothing/under/trek_digi.dmi'

/*
*	SHOES
*/

// SNEAKERS

/datum/greyscale_config/sneakers/worn/digi
	name = "Worn Sneakers - Digitigrade"
	icon_file = 'local/icons/mob/clothing/greyscale/sneakers_digi.dmi'

/datum/greyscale_config/sneakers_orange/worn/digi
	name = "Worn Orange Sneakers - Digitigrade"
	icon_file = 'local/icons/mob/clothing/greyscale/sneakers_digi.dmi'

/datum/greyscale_config/sneakers/worn/vox
	name = "Worn Sneakers - Vox"
	icon_file = 'local/icons/mob/clothing/greyscale/sneakers_vox.dmi'

/datum/greyscale_config/sneakers_orange/worn/vox
	name = "Worn Orange Sneakers - Vox"
	icon_file = 'local/icons/mob/clothing/greyscale/sneakers_vox.dmi'

/datum/greyscale_config/sneakers/worn/teshari
	name = "Worn Sneakers - Teshari"
	icon_file = 'local/icons/mob/clothing/greyscale/sneakers_teshari.dmi'

/datum/greyscale_config/sneakers_orange/worn/teshari
	name = "Worn Orange Sneakers - Teshari"
	icon_file = 'local/icons/mob/clothing/greyscale/sneakers_teshari.dmi'

// Boots

/datum/greyscale_config/boots
	name = "Colorable Boots"
	icon_file = 'local/icons/mob/clothing/greyscale/shoes/shoes.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/shoes/boots/boots.json'

/datum/greyscale_config/boots/worn
	name = "Colorable Boots (Worn)"
	json_config = 'local/code/datums/greyscale/json_configs/shoes/boots/boots_worn.json'

/datum/greyscale_config/boots/worn/digi
	icon_file = 'local/icons/mob/clothing/greyscale/shoes/shoes_digi.dmi'

/datum/greyscale_config/boots/worn/teshari
	icon_file = 'local/icons/mob/clothing/greyscale/shoes/shoes_teshari.dmi'

/datum/greyscale_config/boots/worn/newvox
	icon_file = 'local/icons/mob/clothing/greyscale/shoes/shoes_newvox.dmi'

/datum/greyscale_config/boots/worn/oldvox
	icon_file = 'local/icons/mob/clothing/greyscale/shoes/shoes_oldvox.dmi'

// Laceups

/datum/greyscale_config/laceup
	name = "Colorable Laceups"
	icon_file = 'local/icons/mob/clothing/greyscale/shoes/shoes.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/shoes/laceups/laceups.json'

/datum/greyscale_config/laceup/worn
	name = "Colorable Laceups (Worn)"
	json_config = 'local/code/datums/greyscale/json_configs/shoes/laceups/laceups_worn.json'

/datum/greyscale_config/laceup/worn/digi
	icon_file = 'local/icons/mob/clothing/greyscale/shoes/shoes_digi.dmi'

/datum/greyscale_config/laceup/worn/teshari
	icon_file = 'local/icons/mob/clothing/greyscale/shoes/shoes_teshari.dmi'

/datum/greyscale_config/laceup/worn/newvox
	icon_file = 'local/icons/mob/clothing/greyscale/shoes/shoes_newvox.dmi'

/datum/greyscale_config/laceup/worn/oldvox
	icon_file = 'local/icons/mob/clothing/greyscale/shoes/shoes_oldvox.dmi'

// Sandals

/datum/greyscale_config/sandals
	name = "Colorable Sandals"
	icon_file = 'local/icons/mob/clothing/greyscale/shoes/shoes.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/shoes/sandals/sandals.json'

/datum/greyscale_config/sandals/worn
	name = "Colorable Sandals (Worn)"
	json_config = 'local/code/datums/greyscale/json_configs/shoes/sandals/sandals_worn.json'

/datum/greyscale_config/sandals/worn/digi
	icon_file = 'local/icons/mob/clothing/greyscale/shoes/shoes_digi.dmi'

/datum/greyscale_config/sandals/worn/teshari
	icon_file = 'local/icons/mob/clothing/greyscale/shoes/shoes_teshari.dmi'

/datum/greyscale_config/sandals/worn/newvox
	icon_file = 'local/icons/mob/clothing/greyscale/shoes/shoes_newvox.dmi'

/datum/greyscale_config/sandals/worn/oldvox
	icon_file = 'local/icons/mob/clothing/greyscale/shoes/shoes_oldvox.dmi'

// WRAPS

/datum/greyscale_config/wraps
	name = "Cloth Wraps"
	icon_file = 'local/icons/mob/clothing/greyscale/shoes.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/wraps/wraps.json'

/datum/greyscale_config/wraps/worn
	name = "Cloth Wraps (Worn)"
	json_config = 'local/code/datums/greyscale/json_configs/wraps/wraps_worn.json'

/datum/greyscale_config/wraps/worn/digi
	name = "Cloth Wraps (Worn Digi)"

// MISC SHOES

/datum/greyscale_config/heels
	name = "High Heels"
	icon_file = 'local/icons/mob/clothing/greyscale/heels.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/shoes/heels/heels.json'

/datum/greyscale_config/heels/worn
	name = "High Heels (Worn)"
	json_config = 'local/code/datums/greyscale/json_configs/shoes/heels/heels_worn.json'

/datum/greyscale_config/heels/worn/digi
	name = "High Heels (Worn Digi)"
	json_config = 'local/code/datums/greyscale/json_configs/shoes/heels/heels_worn_digi.json'

/datum/greyscale_config/fancyheels
	name = "Fancy Heels"
	icon_file = 'local/icons/mob/clothing/greyscale/fancyheels.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/shoes/fancyheels/fancyheels.json'

/datum/greyscale_config/fancyheels/worn
	name = "Fancy Heels (Worn)"
	json_config = 'local/code/datums/greyscale/json_configs/shoes/fancyheels/fancyheels_worn.json'

/datum/greyscale_config/fancyheels/worn/digi
	name = "Fancy Heels (Worn Digi)"
	json_config = 'local/code/datums/greyscale/json_configs/shoes/fancyheels/fancyheels_worn_digi.json'

// TESHARI FALLBACKS.

/datum/greyscale_config/teshari
	name = "Teshari Clothing"
	icon_file = 'local/icons/mob/clothing/greyscale/teshari_fallbacks.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/teshari_fallbacks/under.json'

/datum/greyscale_config/teshari/under
	name = "Teshari Under"
	json_config = 'local/code/datums/greyscale/json_configs/teshari_fallbacks/under.json'

/datum/greyscale_config/teshari/under_skirt
	name = "Teshari Skirt"
	json_config = 'local/code/datums/greyscale/json_configs/teshari_fallbacks/under_skirt.json'

/datum/greyscale_config/teshari/shoes
	name = "Teshari Shoes"
	json_config = 'local/code/datums/greyscale/json_configs/teshari_fallbacks/shoes.json'

/datum/greyscale_config/teshari/gloves
	name = "Teshari Gloves"
	json_config = 'local/code/datums/greyscale/json_configs/teshari_fallbacks/gloves.json'

/datum/greyscale_config/teshari/glasses
	name = "Teshari Glasses"
	json_config = 'local/code/datums/greyscale/json_configs/teshari_fallbacks/glasses.json'

/datum/greyscale_config/teshari/coat
	name = "Teshari Coat"
	json_config = 'local/code/datums/greyscale/json_configs/teshari_fallbacks/coat.json'

/datum/greyscale_config/teshari/armor
	name = "Teshari Armor"
	json_config = 'local/code/datums/greyscale/json_configs/teshari_fallbacks/armor.json'

/datum/greyscale_config/teshari/backpack
	name = "Teshari Backpack"
	json_config = 'local/code/datums/greyscale/json_configs/teshari_fallbacks/backpack.json'

/datum/greyscale_config/teshari/spacesuit
	name = "Teshari Spacesuit"
	json_config = 'local/code/datums/greyscale/json_configs/teshari_fallbacks/spacesuit.json'

/datum/greyscale_config/teshari/cloak
	name = "Teshari Cloak"
	json_config = 'local/code/datums/greyscale/json_configs/teshari_fallbacks/cloak.json'

/datum/greyscale_config/teshari/tie
	name = "Teshari Tie"
	json_config = 'local/code/datums/greyscale/json_configs/teshari_fallbacks/tie.json'

/datum/greyscale_config/teshari/scarf
	name = "Teshari Scarf"
	json_config = 'local/code/datums/greyscale/json_configs/teshari_fallbacks/scarf.json'

/datum/greyscale_config/teshari/modcontrol
	name = "Teshari Modsuit"
	json_config = 'local/code/datums/greyscale/json_configs/teshari_fallbacks/modcontrol.json'

/datum/greyscale_config/teshari/hardsuit
	name = "Teshari Hardsuit"
	json_config = 'local/code/datums/greyscale/json_configs/teshari_fallbacks/hardsuit.json'

/datum/greyscale_config/sneakers_marisa/worn/digi
	name = "Marisa Sneakers (Worn Digi)"
	icon_file = 'local/icons/mob/clothing/greyscale/sneakers_digi.dmi'

/*
*	ITEMS
*/

/datum/greyscale_config/id_card
	name = "Colourable ID Card"
	icon_file = 'local/icons/mob/clothing/greyscale/id_card.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/items/id_card.json'

/*
*	AIRLOCKS
*/
/// Generic
/datum/greyscale_config/airlock_effigy
	name = "Effigy Airlock"
	icon_file = 'local/icons/obj/doors/airlocks/effigy/effigy.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/airlock/effigy_airlock_v2.json'

/// Material
/datum/greyscale_config/airlock_effigy/material
	name = "Material Effigy Airlock"
	icon_file = 'local/icons/obj/doors/airlocks/effigy/effigy.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/airlock/effigy_airlock_material.json'

/datum/greyscale_config/airlock_effigy/atmos
	name = "Effigy Airlock - Atmos"
	icon_file = 'local/icons/obj/doors/airlocks/effigy/effigy.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/airlock/atmos.json'

/datum/greyscale_config/airlock_effigy/command
	name = "Effigy Airlock - Command"
	icon_file = 'local/icons/obj/doors/airlocks/effigy/effigy.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/airlock/command.json'

/datum/greyscale_config/airlock_effigy/engineering
	name = "Effigy Airlock - Engineering"
	icon_file = 'local/icons/obj/doors/airlocks/effigy/effigy.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/airlock/engineering.json'

/datum/greyscale_config/airlock_effigy/medical
	name = "Effigy Airlock - Medical"
	icon_file = 'local/icons/obj/doors/airlocks/effigy/effigy.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/airlock/medical.json'

/datum/greyscale_config/airlock_effigy/cargo
	name = "Effigy Airlock - Cargo"
	icon_file = 'local/icons/obj/doors/airlocks/effigy/effigy.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/airlock/cargo.json'

/datum/greyscale_config/airlock_effigy/science
	name = "Effigy Airlock - Science"
	icon_file = 'local/icons/obj/doors/airlocks/effigy/effigy.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/airlock/science.json'

/datum/greyscale_config/airlock_effigy/security
	name = "Effigy Airlock - Security"
	icon_file = 'local/icons/obj/doors/airlocks/effigy/effigy.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/airlock/security.json'

/datum/greyscale_config/airlock_effigy/viro
	name = "Effigy Airlock - Virology"
	icon_file = 'local/icons/obj/doors/airlocks/effigy/effigy.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/airlock/viro.json'

/// Effigy-Exclusives
/datum/greyscale_config/airlock_effigy/service
	name = "Effigy Airlock - Service"
	icon_file = 'local/icons/obj/doors/airlocks/effigy/effigy.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/airlock/service.json'

/datum/greyscale_config/airlock_effigy/studio
	name = "Effigy Airlock - Studio"
	icon_file = 'local/icons/obj/doors/airlocks/effigy/effigy.dmi'
	json_config = 'local/code/datums/greyscale/json_configs/airlock/studio.json'
