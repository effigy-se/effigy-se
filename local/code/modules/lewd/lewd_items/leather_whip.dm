...
/obj/item/clothing/mask/leatherwhip
	...
	worn_icon = 'local/icons/lewd/mob/lewd_clothing/lewd_masks.dmi'
...
/obj/item/clothing/mask/leatherwhip/handle_speech(datum/source, list/speech_args)
	...
	playsound(loc, pick('local/sound/effects/lewd/under_moan_f1.ogg',
...
/obj/item/clothing/mask/leatherwhip/equipped(mob/target, slot)
	...
	whip_overlay = mutable_appearance('local/icons/lewd/mob/lewd_clothing/lewd_masks.dmi', "[base_icon_state]_[current_whip_form]", ABOVE_MOB_LAYER + 0.1) //two arguments. Yes, all mob layer. Fuck person who was working on genitals, they're working wrong.ABOVE_NORMAL_TURF_LAYER
...
//and there is code for successful check, so we are whipping someone
	...
	switch(user.zone_selected) //to let code know what part of body we gonna whip
		...
		if(BODY_ZONE_HEAD)
			...
			target.adjust_pain(5)
			playsound(loc, 'local/sound/effects/lewd/latex.ogg', 80)
