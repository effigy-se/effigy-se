...
/datum/emote/living/mark_turf/run_emote(mob/living/user, params, type_override, intentional)
	...
	if(ishuman(user))
		//feet
		if(!(human_user.bodytype & BODYSHAPE_DIGITIGRADE) && !(human_user.dna.species.mutant_bodyparts["taur"]))
	...
	for(var/choice in user.allowed_turfs)
		...
		option.image = image(icon = 'local/icons/effects/turf_effects_icons.dmi', icon_state = initial(choice))
