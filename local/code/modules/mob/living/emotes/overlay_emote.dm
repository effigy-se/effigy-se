/datum/emote
	var/overlay_emote = 'local/icons/effects/overlay_effects.dmi'
...
/datum/emote/living/sweatdrop/run_emote(mob/living/user, params, type_override, intentional)
	...
	if(isliving(user) && get_toggle(user))
		...
		playsound(get_turf(user), 'local/sound/emotes/generic/sweatdrop.ogg', 25, TRUE)
...
/datum/emote/living/question/run_emote(mob/living/user, params, type_override, intentional)
	...
	if(isliving(user) && get_toggle(user))
		...
		overlay.pixel_y = 28
		user.flick_overlay_static(overlay, 50)
		playsound(get_turf(user), 'local/sound/emotes/generic/question.ogg', 25, TRUE)
...
/datum/emote/living/realize/run_emote(mob/living/user, params, type_override, intentional)
	...
	if(isliving(user) && get_toggle(user))
		...
		if(isteshari(user))
			...
			overlay.pixel_y = 15
		user.flick_overlay_static(overlay, 50)
		playsound(get_turf(user), 'local/sound/emotes/generic/realize.ogg', 25, TRUE)
