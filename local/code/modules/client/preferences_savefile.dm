 ...
 * If the return value is higher than 0, update_character_effigy() will be called later.
 */
/datum/preferences/proc/savefile_needs_update_effigy(list/save_data)
...
/datum/preferences/proc/load_character_effigy(list/save_data)
	if(!save_data)
		save_data = list()
	...
	if(!tgui_prefs_migration)
		...
		")))
		migrate_effigy(save_data)
		addtimer(CALLBACK(src, PROC_REF(check_migration)), 10 SECONDS)

	headshot = save_data["headshot"]

	if(needs_update >= 0)
		update_character_effigy(needs_update, save_data) // needs_update == savefile_version if we need an update (positive integer)

/// Brings a savefile up to date with modular preferences. Called if savefile_needs_update_effigy() returned a value higher than 0
/datum/preferences/proc/update_character_effigy(current_version, list/save_data)
...
/datum/preferences/proc/save_character_effigy(list/save_data)
